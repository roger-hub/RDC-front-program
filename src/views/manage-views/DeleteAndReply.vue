...
<div class="box clearfix">
    ...
    <ul class="messageList">
        <EachMessage @delete="deleteMessage" @submit="submit(arguments)" v-for="(item, index) in list" :key="index" :id="ids[index]">
...
<script>
import EachMessage from "@/components/manage-component/EachMessage";
...
export default {
    ...
    methods: {
        deleteMessage(target) {
            ...
            console.log(id);
            this.$axios.post("/first/message/deleteMessage", {
                messageId: id,
            });
        },
        submit(params) {
        ...
        getMessage() {
            this.$axios
                .post("/first/message/getAllMessage", {})
                .then((res) => {
                    const arr = res.data;
                    this.list = arr.map((item) => {
                        return {
                            ...
                            time: new Date(item.messageTime).getFullYear() +
                                "-" +
                                (new Date(item.messageTime).getMonth() + 1) +
                                "-" +
                                new Date(item.messageTime).getDate(),
                        };
                    });
